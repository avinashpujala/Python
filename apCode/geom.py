...
def fitBSpline(curve, k:int = 3, smoothness = 0.2, n =None, tRange = (0, 1),
               bc_type:str = 'clamped'):
    """
...
def smoothen_curve(points, order=3, smooth_perc=0.1, smooth_fixed=None,
                   wts=None):
    """
    ...
    ** From the good sam xdze2 at stackoverflow.com
    ...
    import numpy as np
    nPts = len(points)
    if smooth_fixed is None:
        smooth = int(smooth_perc*nPts)
    else:
        smooth = smooth_fixed
    dist = np.cumsum(np.sqrt(np.sum(np.diff(points, axis=0)**2, axis=1 )))
    dist = np.insert(dist, 0, 0)/dist[-1]
