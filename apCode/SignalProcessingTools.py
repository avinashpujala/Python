...
class AlignTimeseries(object):
    ...
    def fit(self, y):
        ...
        y: array of shape (N,T) or list of len (N)
            Collection of signals to compute best correlation parameters for. If array,
            N = number of timeseries, and T = lengths of the timeseries. If list, 
            then the N timeseries can be of varying length.
        ...
        -------
        self: class object
            Class object With the following added attributes that can be used f
            or alignment of signals to reference using the method self.transform
        ...
        corr = lambda a,b: correlate(a,b,mode = 'full')/(np.linalg.norm(a)*np.linalg.norm(b))
        ...
        if isinstance(y, np.ndarray):
            if np.ndim(y) == 1:
                y = y[np.newaxis,:]                
        correlogram, lag, shift, polarity = zip(*dask.compute(*[dask.delayed(get_corr_params)(self.ref,y_, self.preserve_polarity) for y_ in y]))
        if isinstance(y, np.ndarray):
    ...
    def transform(self, y):
        """ 
        Applies the best correlation parameters computed by self.fit to transform
        the timeseries in y.
        """
        import numpy as np
        import dask
...
class linalg(): 
    ...
    def orthogonalize(v1,v2):
        ...
        V_perp = [[vec2-(vec1*num[vNum1,vNum2]/den[vNum1]) for vNum2, vec2 in enumerate(v2.T)] for vNum1, vec1 in enumerate(v1.T)]
    ...
    def project(v1,v2):
        ...
        num= np.dot(v1.T,v2)
        den = np.diag(np.dot(v1.T,v1))
        V_proj = [[vec1*num[vNum1,vNum2]/den[vNum1] for vNum2, vec2 in enumerate(v2.T)] for vNum1, vec1 in enumerate(v1.T)]
...
class stats(object):
    ...
    def standardError(Y, Y_est, X):
        ...
        se : (nTargets,nFeatures)
            Standard error, which when the coefficients matrix (regression betas)
            yields the T-values
        """
        import numpy as np
        nSamples = len(Y)
...
#        Sxx = (np.var(X,axis = 0)*nSamples).reshape(1,-1)
#        Sxx= np.sum((X-X.mean(axis = 0))**2,axis = 0).reshape(1,-1)
        Sxx = np.linalg.inv(np.dot(X.T, X)).diagonal().reshape(1,-1)
#        foo = mse@(1/Sxx)
        se_ = np.sqrt(mse@Sxx)
        return se_
